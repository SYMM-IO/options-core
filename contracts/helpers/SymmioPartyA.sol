...
contract SymmioPartyA is AccessControl, IERC1271 {
	 ...
	 */
	constructor(address admin, address multiAccountAddress_, address symmioAddress_, address signatureVerifier_) {
	...
	function isValidSignature(bytes32 hash, bytes calldata signature) external view override returns (bytes4) {
		return
			ISignatureVerifier(signatureVerifier).isValidSignatureEIP1271(hash, signature, IMultiAccount(multiAccountAddress).owners(address(this)));
	}
}
