...
contract SymmioPartyB is Initializable, PausableUpgradeable, AccessControlEnumerableUpgradeable, IERC1271 {
	...
	function initialize(address admin, address symmioAddress_, address signatureVerifier_) public initializer {
		__Pausable_init();
	...
	function isValidSignature(bytes32 hash, bytes calldata signature) external view override returns (bytes4) {
		return ISignatureVerifier(signatureVerifier).isValidSignatureEIP1271(hash, signature, signer);
	}
}
