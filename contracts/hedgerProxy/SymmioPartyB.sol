...
contract SymmioPartyB is Initializable, PausableUpgradeable, AccessControlEnumerableUpgradeable, IERC1271 {
    ...
    function initialize(address admin, address symmioAddress_) public initializer {
        ...
        _grantRole(SETTER_ROLE, admin);//TODO: two other roles are not assigned
    ...
    function isValidSignature(
        ...
        // Validate signatures
        if (_recoverSigner(_hash, _signature) == signer) {
            return 0x1626ba7e;
        } else {
            return 0xffffffff;
        }
    ...
    /// @dev Uses the ecrecover EVM opcode to extract the signer address.
    /// @param _ethSignedMessageHash The Ethereum signed message hash.
    /// @param _signature The signature bytes (r, s, v).
    /// @return recoveredSigner The address that signed the message.
