...
library AccountFacetImpl {
    ...
    function claimWithdraw(uint256 id) internal {
        ...
        require(
            withdrawObject.status == WithdrawStatus.INITIATED,
            "AccountFacet: Already withdrawn"
        );
        if (AppStorage.layout().partyBConfigs[withdrawObject.user].isActive) {
            require(
                block.timestamp >=
                    AppStorage.layout().partyBDeallocateCooldown +
                        withdrawObject.timestamp,
                "AccountFacet: Cooldown hasn't reached"
            );
        } else {
            require(
                block.timestamp >=
                    AppStorage.layout().partyADeallocateCooldown +
                        withdrawObject.timestamp,
                "AccountFacet: Cooldown hasn't reached"
            );
        }
