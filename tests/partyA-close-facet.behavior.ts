...
export function shouldBehaveLikePartyACloseFacet(): void {
	...
	describe("sendCloseIntent", async function () {
		...
		it("Should fail when partyA actions paused", async function () {
			...
			const latestBlock = await getLatestBlockTime()
			await expect(partyA2.sendCloseIntent(1, 100, 7, latestBlock + 140)).to.be.revertedWithCustomError(
		...
		it("Should fail when global paused", async function () {
			...
			const latestBlock = await getLatestBlockTime()
			await expect(partyA2.sendCloseIntent(1, 100, 7, latestBlock + 140)).to.be.revertedWithCustomError(context.partyACloseFacet, "GlobalPaused")
		})

		it("Should fail when msgSender not be PartyA", async function () {
			const latestBlock = await getLatestBlockTime()
			await expect(partyA1.sendCloseIntent(3, 100, 7, latestBlock + 140)).to.be.revertedWithCustomError(
				context.partyACloseFacet,
				"UnauthorizedSender",
			)
		})
		...
		it("Should fail when instant action mode is active", async function () {
			...
			const latestBlock = await getLatestBlockTime()
			await expect(partyA1.sendCloseIntent(1, 100, 7, latestBlock + 140)).to.be.revertedWithCustomError(context.partyACloseFacet, "InstantModeActive")
		})
		...
		it("Should fail when Trade in Invalid state", async function () {
			const latestBlock = (await getLatestBlockTime()) + 100

			await partyA1.sendCloseIntent(1, e(100), 7, latestBlock)
			await partyB1.fillCloseIntent(1, e(100), 7)

			await expect(partyA1.sendCloseIntent(1, 1, 7, latestBlock)).to.be.revertedWithCustomError(context.partyACloseFacet, "InvalidState")
		})

		it("Should fail when deadline low", async function () {
			const latestBlock = await getLatestBlockTime()
			await expect(partyA1.sendCloseIntent(1, 100, 7, latestBlock)).to.be.revertedWithCustomError(context.partyACloseFacet, "LowDeadline")
		})

		it("Should fail when invalid quantity", async function () {
			const latestBlock = await getLatestBlockTime()
			await partyA1.sendCloseIntent(1, e(5), 7, latestBlock + 140)
			await expect(partyA1.sendCloseIntent(1, e(96), 7, latestBlock + 140)).to.be.revertedWithCustomError(context.partyACloseFacet, "InvalidQuantity")
		})
		...
		it("Should fail when invalid Close Intent Count", async function () {
			...
			const latestBlock = await getLatestBlockTime()
			await partyA1.sendCloseIntent(1, 5, 7, latestBlock + 140)
