...
async function main() {
	...
	if (config.balanceLimitPerUser) {
		for (const { collateral, limit } of config.balanceLimitPerUser) {
			await executeAndWait(
				controlFacet.connect(owner).setBalanceLimitPerUser(collateral, limit),
				`Balance Limit Per User configuration. Collateral: ${collateral}, Limit: ${limit}`,
			)
		}
	}
	...
	if (config.timingParameters) {
		await executeAndWait(
			controlFacet
				.connect(owner)
				.setTimingParameters(
					config.timingParameters.partyADeallocateCooldown,
					config.timingParameters.partyBDeallocateCooldown,
					config.timingParameters.forceCancelOpenTimeout,
					config.timingParameters.forceCancelCloseTimeout,
					config.timingParameters.settlementPriceSigValidTime,
					config.timingParameters.upnlSigValidTime,
					config.timingParameters.partyBExclusiveWindow,
					config.timingParameters.unbindingCooldown,
					config.timingParameters.deactiveInstantActionModeCooldown,
				),
	...
	if (config.symbols) {
		...
		const symbolsForAdd = config.symbols.filter((symbol: { name: unknown }) => !oldSymbolNames.has(symbol.name)) // Keep only new symbols
		...
		if (symbolsForAdd.length > BATCH_SIZE) {
			...
			for (let i = 0; i < symbolsForAdd.length; i += BATCH_SIZE) {
				...
				symbols_added_success =
					symbols_added_success &&
					(await executeAndWait(controlFacet.connect(owner).addSymbols(batch), `Symbol batch addition. Batch size: ${batch.length}`))
			}

			if (symbols_added_success)
