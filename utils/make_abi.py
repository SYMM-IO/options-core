...
def process_specific_contract(
    directory: str, contract_name: str, output_filename: str, description: str
):
    ...
    # Look for the contract in the helpers directory
    base_path = os.path.join("artifacts", "contracts", directory, contract_name)
...
def main():
    ...
    multiaccount_count = process_specific_contract(
        "helpers", "MultiAccount.sol", "multiAccount.json", "MultiAccount Contract"
    )

    # Process SymmioPartyB contract specifically
    partyb_count = process_specific_contract(
        "helpers", "SymmioPartyB.sol", "partyB.json", "SymmioPartyB Contract"
    )

    # Process InstantLayer contract specifically
    instant_layer_count = process_specific_contract(
        "helpers", "InstantLayer.sol", "instantLayer.json", "InstantLayer Contract"
    )
    ...
    print(f"Total unique ABI entries in instantLayer.json: {instant_layer_count}")
